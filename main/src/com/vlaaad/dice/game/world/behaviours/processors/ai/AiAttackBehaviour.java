 ...
 */
public class AiAttackBehaviour extends RequestProcessor<Creature, AbilityCreatureParams> {
    @Override public int preProcess(AbilityCreatureParams params) {
        if (params.ability.action instanceof Attack)
            return 2;
        return -1;
    }

    @Override public IFuture<Creature> process(AbilityCreatureParams params) {
        Attack attack = (Attack) params.ability.action;
