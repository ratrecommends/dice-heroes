 ...
 */
public class AiMageTurnProcessor extends AiDefaultTurnProcessor {

    private static final Vector2 tmp1 = new Vector2();

    @Override public float getValue(Creature creature, int x, int y) {
        return getMageValue(creature, x, y);
    }

    public static float getMageValue(Creature creature, int x, int y) {
        World world = creature.world;
        int shotDistance = 4;
        int nearEnemiesCount = Attack.findTargets(creature, world).size;
