 ...
 */
public class RandomAbilityProcessor extends RequestProcessor<Ability,AbilityAbilityParams> {
    @Override public int preProcess(AbilityAbilityParams params) {
        return 1;
    }

    @Override public IFuture<Ability> process(final AbilityAbilityParams params) {
        ...
        params.creature.world.stage.addAction(Actions.delay(
            ...
            Actions.run(new Runnable() {
                @Override public void run() {
                    future.happen(params.creature.world.getController(RandomController.class).random(params.availableAbilities));
                }
            })
        ));
        return future;
