 ...
 */
public class AiShotBehaviour extends RequestProcessor<Creature, AbilityCreatureParams> {
    @Override public int preProcess(AbilityCreatureParams params) {
        if (params.ability.action instanceof Shot)
            return 2;
        return -1;
    }

    @Override public IFuture<Creature> process(AbilityCreatureParams params) {
        Shot attack = (Shot) params.ability.action;
