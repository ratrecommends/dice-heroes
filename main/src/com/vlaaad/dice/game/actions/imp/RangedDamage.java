...
public class RangedDamage extends CreatureAction {
    ...
    public RangedDamage(Ability owner) {
        super(owner);
    }

    @Override public void fillDescriptionParams(Thesaurus.Params params, Creature creature) {
        params.with("radius", String.valueOf(radius)).with("level", String.valueOf(level));
    }
    ...
    @Override protected void doInit(Object setup) {
        ...
        level = MapHelper.get(map, "level", Numbers.ONE).intValue();
    }

    @Override public boolean canBeApplied(Creature creature, Thesaurus.LocalizationData reasonData) {
        return super.canBeApplied(creature, reasonData) && hasTargetsInRadius(creature, reasonData);
    }

    private boolean hasTargetsInRadius(Creature creature, Thesaurus.LocalizationData reasonData) {
        ...
        if (Shot.findTargets(creature, Creature.CreatureRelation.enemy, radius).size > 0)
            return true;
    ...
    @Override public IFuture<? extends IActionResult> apply(final Creature creature, World world) {
        ...
        if (targets.size == 0) {
            ...
            world.getController(BehaviourController.class)
                .get(creature)
                .request(BehaviourRequest.CREATURE, new AbilityCreatureParams(creature, this.owner, targets))
    ...
    private IActionResult calcResult(Creature creature, Creature target) {
        int defenceLevel = target.get(Attribute.defenceFor(AttackType.weapon));
        creature.set(Attribute.attackFor(AttackType.weapon), level);
        int realAttackLevel = creature.get(Attribute.attackFor(AttackType.weapon));
        if (defenceLevel < realAttackLevel) { //success attack
            return new SequenceResult(
                new RangedDamageResult(true, creature, target, level, owner),
                new GiveExpResult(creature, ExpHelper.expForKill(creature, target))
            );
        } else {
            return new SequenceResult(
                new RangedDamageResult(false, creature, target, level, owner),
