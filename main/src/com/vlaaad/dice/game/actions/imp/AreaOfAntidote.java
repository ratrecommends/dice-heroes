 ...
 */
public class AreaOfAntidote extends CreatureAction {
    ...
    public AreaOfAntidote(Ability owner) {
        super(owner);
    }
    ...
    @Override public IFuture<? extends IActionResult> apply(Creature creature, World world) {
        ...
        if (targets.size == 0) {
            ...
            return Future.completed(calcResult(creature, targets));
        }
    }

    private IActionResult calcResult(Creature creature, Array<Creature> targets) {
        Array<IActionResult> results = new Array<IActionResult>(targets.size);
        for (Creature target : targets) {
            results.add(new AntidoteResult(owner, creature, target));
        }
        return new SequenceResult(
            new ParallelResult(results),
            new GiveExpResult(creature, ExpHelper.MIN_EXP * targets.size)
        );
    }
