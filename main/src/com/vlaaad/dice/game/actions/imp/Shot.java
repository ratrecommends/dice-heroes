 ...
 */
public class Shot extends CreatureAction {
    ...
    @Override public IFuture<IActionResult> apply(final Creature creature, World world) {
        Array<Creature> nearEnemies = Attack.findTargets(creature, world);
    ...
    public static IActionResult calcResult(Ability owner, Creature creature, Creature target, AttackType attackType, int attackLevel) {
        ...
        if (defenceLevel < realAttackLevel) { //success attack
            return new SequenceResult(
                new ShotResult(true, creature, target, attackType, attackLevel, owner),
                new GiveExpResult(creature, ExpHelper.expForKill(creature, target))
            );
        }
        return new SequenceResult(
            new ShotResult(false, creature, target, attackType, attackLevel, owner),
            new GiveExpResult(target, ExpHelper.expForDefence(creature, target))
        );
    }

    public static Array<Creature> findTargets(Creature creature, Creature.CreatureRelation relation, float distance) {
        return findTargets(creature, relation, creature.getX(), creature.getY(), creature.world, distance);
    }
