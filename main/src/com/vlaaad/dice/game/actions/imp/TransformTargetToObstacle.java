 ...
 */
public class TransformTargetToObstacle extends CreatureAction {

    private float radius;
    private int turnCount;
    private String obstacle;

    public TransformTargetToObstacle(Ability owner) {
        super(owner);
    }
    ...
    @Override public boolean canBeApplied(Creature creature, Thesaurus.LocalizationData reasonData) {
        return super.canBeApplied(creature, reasonData) && hasTargetsNear(creature, reasonData, radius);
    }

    public static boolean hasTargetsNear(Creature creature, Thesaurus.LocalizationData reasonData, float radius) {
    ...
    @Override public IFuture<? extends IActionResult> apply(final Creature creature, World world) {
        ...
        if (targets.size == 0) {
            ...
            creature.world
                .getController(BehaviourController.class)
                .get(creature)
    ...
    private IActionResult calcResult(Creature creature, Creature target) {
        return new TransformToObstacleResult(creature, target, owner, obstacle, turnCount);
    }
}
