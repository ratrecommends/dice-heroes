 ...
 */
public class EarnedItem extends AchievementCondition<EarnEvent> {
    private final AllOfFilter<Item> filter = new AllOfFilter<Item>();

    public EarnedItem() {
        super(EventType.earnItem);
    }

    @Override public void setup(Object params) {
        Map map = (Map) params;
        ConditionUtil.initItemFilters(filter, map);
    }

    @Override protected boolean satisfied(EarnEvent event) {
        return filter.accept(event.item());
    }
}
