 ...
 */
public class Kill extends AchievementCondition<KillEvent> {
    ...
    @Override public void setup(Object params) {
        Map map = (Map) params;
        mode = Mode.valueOf(MapHelper.get(map, "mode", Mode.exists.toString()));
        relation = Creature.CreatureRelation.valueOf(MapHelper.get(map, "relation", Creature.CreatureRelation.none.toString()));
    }

    @Override protected boolean satisfied(KillEvent event) {
        switch (mode) {
            case exists:
                for (Creature c : event.killed()) {
                    if (event.killer().inRelation(relation, c))
                        return true;
                }
                return false;
            case forall:
