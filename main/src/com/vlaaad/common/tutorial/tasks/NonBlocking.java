 ...
 */
public class NonBlocking extends TutorialTask {
    private final TutorialTask task;

    public NonBlocking(TutorialTask task) {
        super();
        this.task = task;
    }

    @Override public void start(Callback callback) {
        task.initialize(tutorial, resources);
        task.start(new Callback() {
